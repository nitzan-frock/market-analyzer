<script lang="ts">
	...
	import type { MacroIndicator } from '../../../generated/prisma/client';
	import {
		calculateRiskScore,
		interpretationColor,
		INDICATOR_LABELS,
	...
	const displayIndicators = $derived(
		ALL_INDICATORS.map((type) => ({
			type,
			label: INDICATOR_LABELS[type],
			data: indicators.find((ind) => ind.indicator === type)
		})).filter((row) => row.data != null) as {
			type: (typeof ALL_INDICATORS)[number];
			label: string;
			data: MacroIndicator;
		}[]
	);
			...
			{#if !editing}
				{#if indicators.length > 0}
					<div class="overflow-x-auto">
						...
						<tbody>
							{#each displayIndicators as { type, label, data } (type)}
								<tr>
				...
				>
					...
					<div class="overflow-x-auto">
						<table class="table table-xs">
							...
							<tbody>
							{#each ALL_INDICATORS as ind (ind)}
								<tr>
